...
- (void)setStarCount:(NSUInteger)starCount {
    ...
    if (_starCount != starCount) {
        ...
        if (self.returnBackChooseStar) {
            
            self.returnBackChooseStar(self.index);
        }
    }
}
...
- (void)drawRect:(CGRect)rect {
    UIImage *norImage = [UIImage imageNamed:@"icon_star02"];
    UIImage *selImage = [UIImage imageNamed:@"icon_star01"];
    // 图片没间隙自己画
